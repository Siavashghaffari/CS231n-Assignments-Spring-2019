...
def rnn_backward(dh, cache):
    ...
    D = cache[0][0].shape[1]
    dx = np.zeros((N,T,D))
    dx[:,-1,:], dnext_h, dWx, dWh, db = rnn_step_backward(dh[:,-1,:], cache[-1])
        
    for t in range(T-1,0,-1):
        dx[:,t-1,:], dnext_h, dWx_t, dWh_t, db_t = rnn_step_backward(dnext_h+dh[:,t-1,:], cache[t-1])
        dWx += dWx_t
        dWh += dWh_t
        db += db_t
    dh0 = dnext_h
...
def lstm_step_backward(dnext_h, dnext_c, cache):
    ...
    dnext_c1 = (1-np.tanh(next_c)**2)*dtanh_next_c
