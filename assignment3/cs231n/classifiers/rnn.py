...
class CaptioningRNN(object):
    ...
    def loss(self, features, captions):
        ...
        #Forward
        out_image = features.dot(W_proj)+b_proj
        out_word, cache_word = word_embedding_forward(captions_in, W_embed)
        if self.cell_type=='rnn':
            h, cache_rnn = rnn_forward(out_word, out_image, Wx, Wh, b)
        else:
            h, cache_lstm = lstm_forward(out_word, out_image, Wx, Wh, b)
    ...
    def sample(self, features, max_length=30):
        ...
        #v,w = W_embed.shape
        x0 = self._start*np.ones((N,1)).astype(int)
        x = W_embed[x0].squeeze()
        prev_h = h0
        prev_c = np.zeros((N,H))
        ...
        for t in range(max_length):
            ...
            highest_idx = scores.argmax(axis=1)
            captions[:,t] = highest_idx
            x,_ = word_embedding_forward(highest_idx, W_embed)
