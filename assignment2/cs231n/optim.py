...
def adam(w, dw, config=None):
    ...
    t+=1
    grad = config['m']
    grad_squared = config['v']
    grad = config['beta1']*grad+(1-config['beta1'])*dw
    grad_squared = config['beta2']*grad_squared+(1-config['beta2'])*dw*dw
    first_unbias = grad/(1-config['beta1']**t)
    second_unbias = grad_squared/(1-config['beta2']**t)
    next_w = w-config['learning_rate']*first_unbias/(np.sqrt(second_unbias)+config['epsilon'])
    config['m'], config['v'], config['t'] = grad, grad_squared, t
