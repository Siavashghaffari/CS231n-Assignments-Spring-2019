...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None, reg=0.0):
        ...
        scores -= np.amax(scores, axis=1).reshape(N,-1) #avoid instability
        ...
        margin_grad = np.exp(scores)/sum_scores.reshape(N,1)
        margin_grad [np.arange(N),y] = -(1-softmax_loss)
        dW2 = h.T.dot(margin_grad)
        margin_grad /= N
        db2 = np.sum(margin_grad,axis=0)
        grad_h = margin_grad.dot(W2.T)
        dW1 = X.T.dot(grad_h*(h>0))
        db1 = np.sum((grad_h*(h>0)), axis=0)
